...
(function () {
  ...
  function showFormStatus(type, message) {
    ...
    status.className = "form-note is-visible " + type;
  ...
  function validateContactForm() {
    ...
    if (!subject || !subject.value.trim() || subject.value.trim().length < 3) {
      setFieldError("subject", "Please enter a subject (at least 3 characters).");
      valid = false;
    } else {
  ...
  function initContactForm() {
    ...
    contactForm.addEventListener("submit", function (event) {
      ...
      // No backend yet — ready for PHP / email service integration.
      // Example: fetch('api/contact.php', { method: 'POST', body: new FormData(contactForm) })
      // showFormStatus(
      //   "success",
      //   "Looks good! Connect a PHP backend or email service to send this message."
      // );
